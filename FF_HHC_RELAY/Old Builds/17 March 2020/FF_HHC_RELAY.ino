...
void receivedTooManyBytesError(int i)
{
  writeToBufClear();
  writeToBuffer[0] = 'R';
  writeToBuffer[1] = 'e';
...
void noResponseError(int i)
{
  ...
  for ( int j = 0; j < nameLength; j++ )
  {
    writeToBuffer[47+j]=lcdBuffer[j];
  }
  writeToBuffer[47+nameLength] = '\n';
  appendFile("/error.txt",writeToBuffer);
}

void errorCodeError(int i)
{
  writeToBufClear();
  writeToBuffer[0] = 'R';
  writeToBuffer[1] = 'e';
  ...
  for ( int j = 0; j < nameLength; j++ )
  {
    writeToBuffer[55+j]=lcdBuffer[j];
  }
  writeToBuffer[55+nameLength] = '\n';
  appendFile("/error.txt",writeToBuffer);
}

void nonNumberError(int i)
{
  writeToBufClear();
  writeToBuffer[0] = 'R';
  writeToBuffer[1] = 'e';
...
void incompleteResponseError(int i)
{
  writeToBufClear();
  writeToBuffer[0] = 'I';
  writeToBuffer[1] = 'n';
...
void serialBusyError(int i)
{
  ...
  for ( int j = 0; j < nameLength; j++ )
  {
    writeToBuffer[51+j]=lcdBuffer[j];
  }
  writeToBuffer[51+nameLength] = '\n';
  appendFile("/error.txt",writeToBuffer);
}

byte waitForSerialDone()
{
  unsigned long t0 = millis(); //timing variables
...
/*
   ...
   communicates with other mcu through serial2 (isolated ethernet).
*/
void readInputs()
{
  int attemptCount = 0;
    ...
    if ( readyToRead )
    {
      ...
      while ( (unsigned long)(t1-t0)<60000 )
      {
        ...
        if ( comBufferIndex == 20 )
        {
          ...
          // make sure there is no more data to receive
          if ( !waitForSerialDone() )
          {
            serialBusyError(i);
            return;
