  ...
  for ( unsigned char i = 0; i < 22; i++ )
  {
    ...
    if ( readyToRead )
    {
      ...
      if (((unsigned long)(t1-t0))>=60000)
      {
        ...
        writeFile("\error.txt",writeToBuffer);
...
{
  if ( inputs[inputID].type == 1 )
  {
    float score = 0.5 - (inputs[inputID].value - inputs[inputID].minValue) / (inputs[inputID].maxValue - inputs[inputID].minValue);
    score = 4 * score * score;
    return score;
  }
  if ( inputs[inputID].type == 2 )
  {
    return inputs[inputID].value;
  }
  if ( inputs[inputID].type == 3 )
  {
...
byte output(int inputID)
{
  ...
  if ( inputs[inputID].type == 2 || inputs[inputID].type == 3 )
  {
...
void turnOnOutputs()
{
  ...
  int triedIndecesIndex = 0; // stores the current index to triedIndeces array
  while ( triedIndecesIndex < 22 )
  {
    triedIndeces[triedIndecesIndex] = 0; // ensure initialized to 0
    // get a maximum score of unused indeces (inputs not checked)
    // skip testing inputs[0] since it is
    // assumed to be the highest before going into the loop
    for ( int i = 1; i < 22; i++ ) 
    {
    ...
    if ( score > 1 )
    {
      ...
      writeToBuffer[nameLength+36] = ' ';
      writeToBuffer[nameLength+37] = 'w';
      writeToBuffer[nameLength+38] = 'i';
      writeToBuffer[nameLength+39] = 't';
      writeToBuffer[nameLength+40] = 'h';
      writeToBuffer[nameLength+41] = ' ';
      lcdBufClear();
      int dataLength = int2str(inputs[triedIndeces[triedIndecesIndex]].value,lcdBuffer,80);
      for ( int i = 0; i < dataLength; i++ )
      {
        writeToBuffer[nameLength+42+i]=lcdBuffer[i];
      }
      writeToBuffer[nameLength+dataLength+42]='\n';
...
void lcdShowInputSettings()
{
  ...
  switch (inputs[inputSelect].type)
  {
    ...
    case 6:
      ...
      break;
    case 2:
      lcdPrint("no available settings for input type digital");
      delay(1000);
      break;
    case 3:
      lcdPrint("no available settings for input type ndigital");
      delay(1000);
      break;
    case 4:
      lcdPrint("no available settings for input type battery");
      delay(1000);
      break;
    default:
